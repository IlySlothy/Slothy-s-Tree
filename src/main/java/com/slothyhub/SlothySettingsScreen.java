...
public class SlothySettingsScreen extends class_437 {
    ...
    public void method_25394(class_332 ctx, int mx, int my, float delta) {
        ...
        DrawHelper.drawText(ctx, field_22793, "SETTINGS  //  SLOTHYHUB", 18, 14, COL_TEXT, false);
        // Back pill
        boolean backHov = mx >= 4 && mx <= 14 && my >= 10 && my <= 30;
        DrawHelper.drawText(ctx, field_22793, backHov ? "<" : "‹", 8, 16, backHov ? COL_TEXT : COL_MUTED, false);

        // Server URL section
        ctx.method_25294(18, 52, field_22789 - 18, 53, COL_BORDER);
        DrawHelper.drawText(ctx, field_22793, "SERVER", 18, 56, COL_MUTED, false);
        DrawHelper.drawText(ctx, field_22793, "Server URL", 18, 108, COL_TEXT, false);
        DrawHelper.drawText(ctx, field_22793, "Where SlothyHub fetches packs from. Leave empty for local-only mode.", 18, 118, COL_MUTED, false);
        ...
        for (Section section : sections) {
            ctx.method_25294(18, rowY, field_22789 - 18, rowY + 1, COL_BORDER);
            DrawHelper.drawText(ctx, field_22793, section.header(), 18, rowY + 4, COL_MUTED, false);
            rowY += 16;
            for (Row row : section.rows()) {
                ...
                int rowBg = hovered ? Ui.withAlpha(COL_PANEL & 0xFFFFFF, 140) : 0;
                if (rowBg != 0) ctx.method_25294(18, rowY - 2, field_22789 - 18, rowY + 22, rowBg);
                DrawHelper.drawText(ctx, field_22793, row.label(), 26, rowY + 4, hovered ? COL_TEXT : Ui.lerpColor(COL_MUTED, COL_TEXT, 0.5f), false);
                if (!row.hint().isEmpty()) DrawHelper.drawText(ctx, field_22793, row.hint(), 26, rowY + 14, COL_MUTED, false);
        ...
        if (statusMessage != null && System.currentTimeMillis() - statusAt < 3000L) {
            ...
            DrawHelper.drawText(ctx, field_22793, statusMessage, field_22789 / 2 - tw / 2, field_22790 - 28, COL_ACCENT, false);
        }
    ...
    private void renderToggle(class_332 ctx, int x, int y, float t, boolean on) {
        ...
        int trackFg = Ui.lerpColor(COL_BORDER, COL_ACCENT, t);
        ctx.method_25294(x, y, x + trackW, y + trackH, trackFg);
        ctx.method_25294(x + 1, y + 1, x + trackW - 1, y + trackH - 1, COL_BG);
        ctx.method_25294(x, y, x + trackW, y + trackH, Ui.withAlpha(COL_ACCENT & 0xFFFFFF, (int)(180 * t)));
        int thumbX = x + 2 + (int)((trackW - 14) * t);
        ctx.method_25294(thumbX, y + 2, thumbX + 10, y + trackH - 2, Ui.lerpColor(COL_BORDER, COL_TEXT, t));
    }

    private void drawFooterBtn(class_332 ctx, int x, int y, int w, int h, String label, boolean hov, boolean danger) {
        int border = danger ? COL_DANGER : (hov ? COL_ACCENT : COL_BORDER);
        int fill = hov ? (danger ? COL_DANGER : COL_ACCENT) : COL_BG;
        int fg = hov ? (danger ? COL_TEXT : -16777216) : (danger ? COL_DANGER : COL_MUTED);
