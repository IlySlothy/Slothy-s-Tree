...
public class SlothyBuilderScreen extends class_437 {
    ...
    private static final int COL_SURFACE = Ui.COL_SURFACE;
    ...
    protected void method_25426() {
        ...
        searchField.method_1868(COL_TEXT);
    ...
    public void method_25394(class_332 ctx, int mx, int my, float delta) {
        ...
        DrawHelper.drawText(ctx, field_22793, "WEAVE PACK  //  SLOTHYHUB", 18, 14, COL_TEXT, false);
    ...
    private void renderStepIndicator(class_332 ctx, float delta) {
        ...
        for (int i = 0; i < STEPS.length; i++) {
            ...
            int dotColor = Ui.lerpColor(COL_BORDER, COL_ACCENT, t);
            ctx.method_25294(dotX - 5, dotY - 5, dotX + 5, dotY + 5, dotColor);
            ctx.method_25294(dotX - 3, dotY - 3, dotX + 3, dotY + 3, i == step ? COL_ACCENT : COL_BG);
            if (i > 0) {
                int lineX = dotX - 32, lineColor = Ui.lerpColor(COL_BORDER, COL_ACCENT, stepT[i - 1]);
                ctx.method_25294(lineX, dotY, dotX - 5, dotY + 1, lineColor);
            }
            String label = STEPS[i];
            DrawHelper.drawText(ctx, field_22793, label, dotX - field_22793.method_1727(label) / 2, dotY + 10, i == step ? COL_TEXT : Ui.lerpColor(COL_MUTED, COL_TEXT, t * 0.5f), false);
        }
    }
    ...
    private void renderStepSelect(class_332 ctx, int mx, int my, float delta) {
        ...
        DrawHelper.drawText(ctx, field_22793, "ADD PACKS TO YOUR WEAVE QUEUE", cardX, 76, COL_MUTED, false);
        if (queue.isEmpty()) {
            DrawHelper.drawText(ctx, field_22793, "Queue is empty — click packs from the list to add them.", cardX, listY + listH / 2, COL_MUTED, false);
        } else {
            int qy = listY;
            for (String packName : queue) {
                ctx.method_25294(cardX, qy, cardX + cardW, qy + 22, COL_PANEL);
                ctx.method_25294(cardX, qy + 21, cardX + cardW, qy + 22, COL_BORDER);
                ctx.method_25294(cardX, qy, cardX + 2, qy + 22, COL_ACCENT);
                DrawHelper.drawText(ctx, field_22793, packName, cardX + 8, qy + 7, COL_TEXT, false);
                boolean rmHov = mx >= cardX + cardW - 22 && mx <= cardX + cardW - 6 && my >= qy + 4 && my <= qy + 18;
                DrawHelper.drawText(ctx, field_22793, "×", cardX + cardW - 16, qy + 7, rmHov ? COL_DANGER : COL_MUTED, false);
                qy += 24;
            }
        }
    }
    ...
    private void renderStepConfigure(class_332 ctx, int mx, int my) {
        DrawHelper.drawText(ctx, field_22793, "CONFIGURE WEAVE OPTIONS", 18, 80, COL_MUTED, false);
        DrawHelper.drawText(ctx, field_22793, "Packs are applied in queue order. First pack wins on conflicts.", 18, 96, COL_MUTED, false);
        int cardW = Math.min(field_22789 - 36, 480), cardX = (field_22789 - cardW) / 2;
        int qy = 116; int i = 0;
        for (String packName : queue) {
            ctx.method_25294(cardX, qy, cardX + cardW, qy + 22, COL_PANEL);
            ctx.method_25294(cardX, qy, cardX + 2, qy + 22, COL_ACCENT);
            DrawHelper.drawText(ctx, field_22793, (i + 1) + ". " + packName, cardX + 8, qy + 7, COL_TEXT, false);
            ...
            DrawHelper.drawText(ctx, field_22793, "▲", cardX + cardW - 40, qy + 7, upHov ? COL_TEXT : COL_MUTED, false);
            DrawHelper.drawText(ctx, field_22793, "▼", cardX + cardW - 18, qy + 7, dnHov ? COL_TEXT : COL_MUTED, false);
            qy += 24; i++;
        }
    }
    ...
    private void renderStepWeave(class_332 ctx, int mx, int my, float delta) {
        ...
        if (building) {
            Ui.spinner(ctx, cx, cy, 8, buildPulseT, COL_ACCENT);
            DrawHelper.drawText(ctx, field_22793, "WEAVING PACKS...", cx - field_22793.method_1727("WEAVING PACKS...") / 2, cy + 20, COL_TEXT, false);
        } else if (buildResult != null) {
            DrawHelper.drawText(ctx, field_22793, buildResult, cx - field_22793.method_1727(buildResult) / 2, cy, COL_TEXT, false);
        } else {
            DrawHelper.drawText(ctx, field_22793, "READY TO WEAVE", cx - field_22793.method_1727("READY TO WEAVE") / 2, cy - 12, COL_TEXT, false);
            DrawHelper.drawText(ctx, field_22793, queue.size() + " packs in queue", cx - field_22793.method_1727(queue.size() + " packs in queue") / 2, cy + 4, COL_MUTED, false);
        }
    }
    ...
    private void drawBtn(class_332 ctx, int x, int y, int w, int h, String label, boolean hov, boolean primary) {
        int border = hov ? COL_ACCENT : COL_BORDER, fill = primary && hov ? COL_ACCENT : (hov ? COL_SURFACE : COL_BG);
        int fg = primary && hov ? -16777216 : (hov ? COL_TEXT : COL_MUTED);
